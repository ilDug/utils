...
var Fuse = /** @class */ (function () {
     ...
     */
    Fuse.prototype._set = function (list) {
        this.list = list;
        return list;
    };
        ...
        if (typeof text === 'string') {
            ...
            if (this.options.tokenize) {
                for (i = 0; i < this.tokenSearchers.length; i++) {
                    ...
                    if (options.verbose)
                        console.log('Pattern:', tokenSearcher.pattern);
                    termScores = [];
                    ...
                    if (options.verbose)
                        console.log('Token scores:', termScores);
                }
                ...
                if (options.verbose)
                    console.log('Token score average:', averageScore);
            }
            mainSearchResult = this.fullSeacher.search(text);
            if (options.verbose)
                console.log('Full text score:', mainSearchResult.score);
            finalScore = mainSearchResult.score;
            if (averageScore !== undefined) {
                finalScore = (finalScore + averageScore) / 2;
            }
            if (options.verbose)
                console.log('Score average:', finalScore);
            checkTextMatches = (this.options.tokenize && this.options.matchAllTokens) ? numTextMatches >= this.tokenSearchers.length : true;
            if (options.verbose)
                console.log('Check Matches', checkTextMatches);
    ...
    // Helpers
    Fuse.prototype.deepValue = function (obj, path, list) {
        var firstSegment;
        var remaining;
        var dotIndex;
        var value;
    ...
    Fuse.prototype.isArray = function (obj) {
        return Object.prototype.toString.call(obj) === '[object Array]';
    };
    return Fuse;
}()); //chiude la classe
export { Fuse };
var BitapSearcher = /** @class */ (function () {
    function BitapSearcher(pattern, options) {
         ...
         * @private
         */
        this._calculatePatternAlphabet = function () {
            var mask = {}, i = 0;
         ...
         * @private
         */
        this._bitapScore = function (errors, location) {
        ...
        this._getMatchedIndices = function (matchMask) {
            var matchedIndices = [];
            var start = -1;
            var end = -1;
            var i = 0;
    ...
    return BitapSearcher;
}()); //chiude la classe
export { BitapSearcher };
