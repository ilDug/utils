...
class PasswordController extends AccountController
{
    ...
    public function recover($email)
    {
        /** EMAIL */
        if (!$email) {
            throw new Exception("l'email assente", 400);
            return;
        }
        ...
        $st->execute();
        if (!$uid = $st->fetch()->uid) {
            throw new Exception("indirizzo email non presente nel database", 400);
            return;
        }
        ...
        if (!$st->execute()) {
            throw new Exception("Errore creazione chiave di recupero", 500);
            return;
        };
