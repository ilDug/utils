...
class AccountController
{
    ...
    public function login($email, $pw)
    {
        ...
        if (!$email) {
            throw new Exception("Email assente", 400);
            return false;
        }
        if (!$pw) {
            throw new Exception("Password assente", 400);
            return false;
        }
        ...
        if (!$o) {
            ...
            if (!password_verify($pw, $o->password)) {
                ...
                $st->execute();
                if (!$u = $st->fetch()) {
                    throw new Exception("Errore ricerca utente", 500);
                    return false;
                }
    ...
    {
        if (!$user) {
            throw new Exception("parametro UTENTE non corretto", 400);
            return;
        }
